...
-목적-
LSTM을 2-layer로 구성하여 영화 리뷰가 긍정인지 부정인지 판별하는 모델 학습

-학습 결과-
훈련 데이터셋에 약 97%, 테스트 데이터셋에 약 82%의 성능을 보였다.
"""
import os
...
class LSTM(tensorflow.keras.Model):
    def __init__(self, units):
        ...
        self.lstm = tensorflow.keras.Sequential([
            tensorflow.keras.layers.LSTM(units=units, dropout=0.2, return_sequences=True, unroll=True),
            tensorflow.keras.layers.LSTM(units=units, dropout=0.2)
        ])
