...
-목적-
LSTMCell을 2-layer로 구성하여 영화 리뷰가 긍정인지 부정인지 판별하는 모델 학습
...
#LSTMCell을 이용한 모델 정의
class LSTM_by_cell(tensorflow.keras.Model):
    def __init__(self, units):
        super(LSTM_by_cell, self).__init__()

        self.state0 = [tensorflow.zeros([batch_size, units]), tensorflow.zeros([batch_size, units])]
        self.state1 = [tensorflow.zeros([batch_size, units]), tensorflow.zeros([batch_size, units])]
        ...
        self.LSTMCell0 = tensorflow.keras.layers.LSTMCell(units=units, dropout=0.2)
        self.LSTMCell1 = tensorflow.keras.layers.LSTMCell(units=units, dropout=0.2)
    ...
    def call(self, inputs, training=None):
        ...
        state0 = self.state0
        state1 = self.state1
        for word in tensorflow.unstack(x, axis=1):
            out0, state0 = self.LSTMCell0(word, state0, training)
            out1, state1 = self.LSTMCell1(out0, state1, training)
