...
-목적-
SimpleRNN을 2-layer로 구성하여 영화 리뷰가 긍정인지 부정인지 판별하는 모델 학습

-학습 결과-
훈련 데이터셋에 약 99%, 테스트 데이터셋에 약 79%의 성능을 보였다.
"""
import os
...
#RNN을 이용한 모델 정의
class RNN(tensorflow.keras.Model):
    def __init__(self, units):
        ...
        self.rnn = tensorflow.keras.Sequential([
            tensorflow.keras.layers.SimpleRNN(units=units, dropout=0.2, return_sequences=True),
            tensorflow.keras.layers.SimpleRNN(units=units, dropout=0.2)
        ])
